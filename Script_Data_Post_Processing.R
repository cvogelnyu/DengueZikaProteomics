### Clean up unnecessary data (reduce file size)
d = read.csv("Spectronaut_Output_Zika_BGS_Report.csv", 
             header=T, as.is=T)

## new Spectronaut version has one additional column in BGS report => remove column 11 (PG.Quantity)
d = d[,-11]

## Make unique identifier to get a single value for a fragment in a peptide
gg = c(1,2,7,11,29,41,42,43,45,46,47,48,49,50,53,55,57)
d = d[, gg]
...
write.table(d, "Zika_Dengue_reduced.txt", sep="\t", quote=F, row.names=F)

## Make sure that there are no replicate fragment ions 
## (by taking maximum intensity of the ions of the same type).
## By type I mean: same peptide sequence, precursor ion charge, fragment ion mz and charge, 
## fragment ion loss type (e.g. H2O)
d = read.delim("Zika_Dengue_reduced.txt", header=T, as.is=T)
...
write.table(dat, "merged_table.txt", sep="\t", quote=F, row.names=F, na="")

#### Quality Control, Filtering

d = read.delim("merged_table.txt", header=T, as.is=T)
...
###
sdata = read.delim("ConditionSetup.txt", header=T, as.is=T)
...
pdf("2.1_boxplot.pdf", height=10, width=25)
par(mar=c(20,5,2,3))
...
LM = 150
pdf("2.2_PCAplot_before_batch_correction.pdf", height=5.5, width=5, useDingbats = FALSE)
...
legend(-LM,LM, c("QC","Dengue","Zika"), pch=19, col=c(1,2,4), cex=0.8)
...
pdf("2.3_fragment_ion_plots.pdf", height=12, width=8, useDingbats = FALSE)
par(mfrow=c(4,1))
for(i in 1:nrow(x)) {
  tmp = as.numeric(x[i,])
  tmp1 = tmp[pt1]
...
pdf("2.4_fragment_ion_plots_post_correction.pdf", height=12, width=8, useDingbats = FALSE)
...
LM = 130
pdf("2.5_PCAplot_after_batch_correction.pdf", height=5.5, width=5, useDingbats = FALSE)
plot(tmp.pca$x[,1], tmp.pca$x[,2], col=sdata$color, pch=ppp,
     xlim=c(-LM,LM), ylim=c(-LM,LM), xlab="PC1 (24%)", ylab="PC2 (5%)", 
     main="Fragment ion peak area data", cex=0.5)
legend(-LM,LM, c("QC","Dengue","Zika"), pch=19, col=c(1,2,4), cex=0.8)
#text(tmp.pca$x[,1], tmp.pca$x[,2], colnames(xx), cex=.4)
abline(v=0, lty=2)
...
####Second boxplot
pdf("2.6_boxplot_after_batch_correction.pdf", height=10, width=25)
par(mar=c(20,5,2,3))
boxplot(log2(tmpdata), las=2, ylab="Log2 Peak Area", cex=.3)
dev.off()
...
### PCA summary 
#0.25 19.64  6.39  4.49  3.45  2.88  2.77


pdf("2.7_PCAplot_after_batch_correction.pdf", height=11, width=10.5, useDingbats = FALSE)
LM = 70
...
ord = order(sdata$Type, sdata$Patient, sdata$TimePoint)
sdata = sdata[ord, ]
m = match(sdata$File.Name, colnames(tmpdata))
...
#0.6 non-missing, lowess 0.25: 303, 3329, 12887
