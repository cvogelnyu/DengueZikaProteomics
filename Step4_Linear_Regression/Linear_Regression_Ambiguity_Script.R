...
sdata = read.delim("sample_meta_data_ambiguity.txt", header=T) #, as.is=T)  ## We need to keep categorial vars as "factors"
...
pdata_a1 <- pdata_trainingset
pdata_a2 <- pdata_trainingset
...
id1 = !is.na(Unambiguous)
id2 = !is.na(Unambiguous) & !is.na(IgM) & !is.na(Gender) & !is.na(Race) & !is.na(Days_since_onset_corrected) & !is.na(Age)
...
pval.unambiguous = rep(NA, np)
coef.unambiguous = rep(NA, np)
...
for(i in 1:np) {
  ...
  tmp.fit = lm(xx ~ Unambiguous) 
  tmp.coef = coef(tmp.fit)
  tmp.res = xx  * tmp.coef[2]
  resdata[i,] = tmp.res
  
  ## record p-values
  pval.unambiguous[i] = summary(tmp.fit)$coef[2,4]
  
  
  coef.unambiguous[i] = summary(tmp.fit)$coef[2,1]
}

pmat = data.frame(Protein=rownames(pdata_a1), 
                  pval_unambiguous=pval.unambiguous, 
                  coef_unambiguous=coef.unambiguous, 
                  stringsAsFactors = FALSE)

pmat[,2] = round(pmat[,2], 5)
pmat[,3] = round(pmat[,3], 2)

write.table(pmat, "pvalue_matrix_AnalysisA_Ambiguity_Training.txt", sep="\t", quote=F, row.names=F)
...
write.csv(pdata2_a1, "Residual_data_AnalysisA_Ambiguity.csv")

#### Significant proteins only
psig = pmat$pval_unambiguous <= 0.01
...
pval.unambiguous = pval.IgM = pval.Gender = pval.Race = pval.Days_since_onset_corrected = pval.Age = rep(NA, np)
coef.unambiguous = coef.IgM = coef.Gender = coef.Race = coef.Days_since_onset_corrected = coef.Age = rep(NA, np)
...
for(i in 1:np) {
  ...
  tmp.fit = lm(xx ~ Unambiguous + IgM + Gender + Race + Days_since_onset_corrected + Age) 
  tmp.coef = coef(tmp.fit)
...
pmat = data.frame(Protein=rownames(pdata_a2), 
                  pval_unambiguous=pval.unambiguous, pval_IgM=pval.IgM, 
                  ...
                  coef_unambiguous=coef.unambiguous, coef_IgM=coef.IgM, 
...
write.table(pmat, "pvalue_matrix_AnalysisB_Ambiguity_Training.txt", sep="\t", quote=F, row.names=F)

## Impute missing data for visualization and clustering
library(impute)
pdata2_a2 = impute.knn(as.matrix(resdata))$data   
### pdata2 object holds the "residual" data after removing confounder effects
write.csv(pdata2_a2, "Residual_data_AnalysisB_Ambiguity.csv")
